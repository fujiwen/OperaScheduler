...
host echo RMAN target sys/opera10g cmdfile d:\scripts\logs\del_app_arch.rcv log d:\scripts\logs\del_app_arch.log > d:\scripts\logs\del_arch_rman.bat

spool d:\scripts\logs\del_app_arch.rcv
...
spool d:\scripts\logs\daily_report.html
...
begin
	...
	if v_status <> 'MOUNTED' then
		DBMS_OUTPUT.put_line('<center><font size="+0" face="Arial,Helvetica,Geneva,sans-serif" color="#FF0000"><b>(STANDBY database must be in MOUNTED state, please run d:\scripts\start_standby.bat)</b></font></center>');
	else null;
	end if;
end;
/
...
from v$archived_log
where applied='NO' and registrar='RFS' and creator='ARCH'
/
set heading off
declare
v_log	number;
begin
	select count(*) into v_log from v$archived_log where applied='NO' and registrar='RFS' and creator='ARCH';
	if v_log > 0 then
...
host d:\scripts\logs\del_arch_rman.bat

host del d:\scripts\logs\del_arch_rman.bat

host del d:\scripts\logs\del_app_arch.rcv
